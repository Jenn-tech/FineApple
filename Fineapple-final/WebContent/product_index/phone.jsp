<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
 <figure class="product">
                <img src="https://images.samsung.com/is/image/samsung/sec-galaxy-z-fold2-f916-sm-f916nznakoo-openbackmysticbronze-308345462?$PD_GALLERY_L_PNG$" width="250px" height="230px" />
                 <figcaption>
                  <h3>  더 알아보기</h3>
                  <h5>more detail</h5>
                 </figcaption>
                <a href="../product/phone1.jsp"></a>
                <div id="detail">갤럭시 Z 폴드2 5G</br>2,398,000</div>
            </figure>

              <figure class="product">
                <img src="https://images.samsung.com/is/image/samsung/sec-galaxy-z-fold2-f916-sm-f916nzkakoo-openbackmysticblack-308345316?$PD_GALLERY_L_PNG$" width="250px" height="230px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone6.jsp"></a>
                <div id="detail">갤럭시 Z 폴드2 5G</br>2,398,000</div>
            </figure>

              <figure class="product">
                <img src="https://images.samsung.com/is/image/samsung/sec-galaxy-z-flip-5g-f707-sm-f707nzwakoo-fronttabletopmysticwhite-308345440?$PD_GALLERY_L_PNG$" width="340px" height="260px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone2-1.jsp"></a>
                <div id="detail">갤럭시 Z 플립 5G</br>1,650,000</br></div>
              </figure>
              
              <figure class="product">
                <img src="https://images.samsung.com/is/image/samsung/sec-galaxy-z-flip-5g-f707-sm-f707nznakoo-frontmysticbronze-308345243?$PD_GALLERY_L_PNG$" width="340px" height="260px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone2.jsp"></a>
                <div id="detail">갤럭시 Z 플립 5G</br>1,650,000</br></div>
              </figure>



              <figure class="product">
                <img src="https://images.samsung.com/is/image/samsung/sec-galaxy-z-flip-5g-f707-sm-f707nzaakoo-frontmysticgray-308345429?$PD_GALLERY_L_PNG$" width="340px" height="260px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone2-2.jsp"></a>
                <div id="detail">갤럭시 Z 플립 5G</br>1,650,000</br></div>
              </figure>

              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone-12-pro-max-blue-hero?wid=940&hei=1112&fmt=png-alpha&qlt=80&.v=1604021658000" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone3.jsp"></a>
                <div id="detail">iPhone 12 Pro Max</br>1,490,000</br></div>
              </figure>

              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone-12-pro-max-gold-hero?wid=940&hei=1112&fmt=png-alpha&qlt=80&.v=1604021660000" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone3-1.jsp"></a>
                <div id="detail">iPhone 12 Pro Max</br>1,490,000</br></div>
              </figure>
              
              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone-12-pro-max-graphite-hero?wid=470&hei=556&fmt=png-alpha&.v=1604021658000" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone3-2.jsp"></a>
                <div id="detail">iPhone 12 Pro Max</br>1,490,000</br></div>
              </figure>

              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone-12-green-select-2020?wid=470&hei=556&fmt=png-alpha&.v=1604343704000" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone4.jsp"></a>
                <div id="detail">iPhone 12</br>1,480,000</br></div>
              </figure>

              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone-12-red-select-2020?wid=470&hei=556&fmt=png-alpha&.v=1604343703000" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone4-1.jsp"></a>
                <div id="detail">iPhone 12</br>1,480,000</br></div>
              </figure>
              
              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone11-yellow-select-2019?wid=470&hei=556&fmt=png-alpha&.v=1568141245782" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone5.jsp"></a>
                <div id="detail">iPhone 11</br>1,060,000</br></div>
              </figure>
              
              <figure class="product">
                <img src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/iphone11-purple-select-2019?wid=470&hei=556&fmt=png-alpha&.v=1566960958082" width="200px" height="250px" />
                <figcaption>
                  <h3>더 알아보기</h3>
                  <h5>more detail</h5>
                </figcaption>
                <a href="../product/phone5-1.jsp"></a>
                <div id="detail">iPhone 11</br>1,060,000</br></div>
              </figure>